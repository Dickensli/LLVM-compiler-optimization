...
class DataFlowAnalysis {
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
		...
		for (auto iter = InstrToIndex.begin(); iter != InstrToIndex.end(); ++iter) {
		...
		while (!worklist.empty()) {
			...
			std::vector<Info*> flow;

			getIncomingEdges(idx, &incoming);
			getOutgoingEdges(idx, &outgoing);
			flowfunction(IndexToInstr[idx], incoming, outgoing, flow);
			for (unsigned i = 0; i < outgoing.size(); i++) {
				Edge outEdge = Edge(idx, outgoing[i]);
				Info* joinFlow = new Info();
				Info::join(EdgeToInfo[outEdge], flow[i], joinFlow);
				if (!Info::equals(EdgeToInfo[outEdge], flow[i])) {
					EdgeToInfo[outEdge] = joinFlow;
